...
const userPropertyServices = require("../services/user_property.service");
const user_property = require("../models/user_property.model");
const pagination = require("../middleware/pagination");
const { protect } = require("../middleware/authMiddleware");
const upload = require("../middleware/upload");


router.get("/searchUserProperty", async (req, res) => {
    ...
    const results = {}
    if (endIndex < await user_property.countDocuments().exec()) {
        results.next = {
            page: page + 1,
            limit: limit
        }
    ...
    console.log(req.query);
    try {
        results.results = await user_property.aggregate([
            {
                $match: {
                    $and: [
        ...
        results.count = await user_property.count()
        console.log(results)
        res.json(results);
    }
    catch (e) {
...
router.get("/getUserProperties", async (req, res) => {
...
router.get("/getUserPropertiesByUserId/:id", (req, res) => {
    console.log("Hello world", req.params.id)
    ...
    user_property.aggregate([
        ...
        .then((properties) => { console.log(properties); res.json({ results: properties }) })
        .catch((err) => res.status(400).json({ error: err }));
});

router.post("/addUserProperty", upload.array("SelectedImages", 5), (req, res) => {
    userPropertyServices
        .addUserProperty(req.body, req.files)
...
router.put("/updateUserProperty/:id", upload.array("SelectedImages", 5), (req, res) => {
    userPropertyServices
        .updateUserProperty(req.params.id, req.body, req.files)
        .then(() => res.json("Property Updated"))
        .catch((err) => res.status(400).json({ error: err }));
});
router.delete("/deleteUserProperty/:id", (req, res) => {
    userPropertyServices
        .deleteUserProperty(req.params.id)
